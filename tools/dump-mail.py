...
PARSER.add_argument('--exec', type=str, help='Run this command on the file (shell execute) file on standard in')
...
def main():
    ...
    executable = dict(args._get_kwargs())['exec']
...
def receive(message, address=None, host=None):
    ...
    if executable:
        ...
        p.communicate(message_text)
        print("Shell {!r} result: {!r}".format(executable, p.returncode))

    if not dump_dir and not executable:
